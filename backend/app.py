...
def update_profile():
    ...
    if 'new_password' in data and data.get('new_password') and 'current_password' in data:
        ...
        else: return jsonify({'error': 'Invalid current password'}), 401
    db.session.commit()
    return jsonify(user.to_dict()), 200
...
class ResourceList(Resource):
    model = None
    def get(self): return [item.to_dict() for item in self.model.query.all()], 200
    def post(self):
        data = request.get_json()
        try:
...
api.add_resource(Tenants, "/api/tenants")
api.add_resource(TenantById, "/api/tenants/<int:id>")
api.add_resource(Leases, "/api/leases")
